...
test("관리자가 테마를 비활성화하면 사용자 테마 목록에서 사라진다", async ({ page }) => {
  const suffix = uniqueSuffix();
  const themeName = `toggle-${suffix}`;

  await page.goto(ADMIN_PATH);
  await createTheme(page, {
    themeName,
    description: `toggle-desc-${suffix}`,
    thumbnailImageUrl: `https://example.com/toggle-${suffix}.png`
  });

  await toggleThemeStatus(page, themeName, "비활성화");

  const adminRow = page.locator('[data-role="theme-table"] tr').filter({ hasText: themeName });
  await expect(adminRow.locator(".td-name")).toHaveClass(/text-gray/);

  await page.goto(RESERVE_PATH);
  await expect(page.locator("#themeSelect option").filter({ hasText: themeName })).toHaveCount(0);

  await page.goto(ADMIN_PATH);
  await toggleThemeStatus(page, themeName, "활성화");
...
async function toggleThemeStatus(page, themeName, targetStatus) {
  const row = page.locator('[data-role="theme-table"] tr').filter({ hasText: themeName });

  await row.getByRole("button", { name: targetStatus }).click();

  await expect(page.locator('[data-role="toast"]')).toContainText(`테마가 ${targetStatus}되었습니다.`);
}
