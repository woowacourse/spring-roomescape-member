...
public class LoginController {
    ...
    public ResponseEntity<MemberNameResponse> checkLogin(@AuthenticationPrincipal String token) {
        LoginMemberResponse loginMemberResponse = service.getLoginMemberByToken(token);
        MemberNameResponse response = new MemberNameResponse(loginMemberResponse.name());
