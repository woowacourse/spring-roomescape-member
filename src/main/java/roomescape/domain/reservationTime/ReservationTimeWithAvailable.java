...
public record ReservationTimeWithAvailable(long id, LocalTime startAt, boolean isAvailable) {
    ...
    public ReservationTimeWithAvailable {
        validateStartAtValue(startAt);
    }

    public ReservationTimeWithAvailable(long id, String startAt, boolean isAvailable) {
        this(id, parseLocalTime(startAt), isAvailable);
    }

    private static void validateStartAtValue(LocalTime startAt) {
        if (startAt == null) {
            throw new InvalidRequestValueException(INVALID_START_TIME_NULL);
        }
    }

    private static LocalTime parseLocalTime(String startAt) {
