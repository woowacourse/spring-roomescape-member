...
public class Theme {
    ...
    private void validate(String name, String description, String imageUrl) {
        ...
        if (name.length() > MAX_VARCHAR_LENGTH) {
            throw new IllegalArgumentException("테마 이름은 " + MAX_VARCHAR_LENGTH + "자를 초과할 수 없습니다.");
        }
        ...
        if (description.length() > MAX_VARCHAR_LENGTH) {
            throw new IllegalArgumentException("테마 설명은 " + MAX_VARCHAR_LENGTH + "자를 초과할 수 없습니다.");
        }

        if (imageUrl == null || imageUrl.isBlank()) {
            throw new IllegalArgumentException("테마 이미지 URL은 비어있을 수 없습니다.");
        }
        if (imageUrl.length() > MAX_URL_LENGTH) {
            throw new IllegalArgumentException("이미지 URL은 " + MAX_URL_LENGTH + "자를 초과할 수 없습니다.");
        }
    }
}
