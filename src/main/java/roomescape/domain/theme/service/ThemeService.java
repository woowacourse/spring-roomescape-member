...
public class ThemeService {
    ...
    public ThemeReservationTimesResponse findAllThemeReservationTimes(Long themeId, LocalDate date) {
        List<ThemeReservationTimeResult> results = themeRepository.findAllThemeReservationTimesByThemeIdAndDate(
                themeId,
                date
        );

        List<ThemeReservationTimeResponse> times = results.stream()
                .map(result -> new ThemeReservationTimeResponse(
                        result.id(),
                        result.startAt(),
                        result.isAvailable()
                ))
    ...
    public PopularThemesResponse findPopularThemes(Integer period, Integer limit) {
        ...
        List<PopularThemeResponse> responses = popularThemes.stream()
                .map(popularTheme -> new PopularThemeResponse(
                        popularTheme.id(),
                        popularTheme.name(),
                        popularTheme.description(),
                        popularTheme.thumbnailUrl(),
                        popularTheme.rank()
                ))
                .toList();
