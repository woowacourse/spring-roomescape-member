...
import roomescape.theme.application.ThemeWebFacade;
...
public class ThemeController {
    ...
    private final ThemeWebFacade themeWebFacade;

    @GetMapping
    public ResponseEntity<List<ThemeResponse>> getAll() {
        return ResponseEntity.ok(themeWebFacade.getAll());
    }

    @GetMapping("/ranking")
    public ResponseEntity<List<ThemeResponse>> getRanking() {
        return ResponseEntity.ok(themeWebFacade.getRanking());
    }

    @PostMapping
    public ResponseEntity<ThemeResponse> create(@RequestBody final CreateThemeWebRequest createThemeWebRequest) {
        final ThemeResponse themeResponse = themeWebFacade.create(createThemeWebRequest);
