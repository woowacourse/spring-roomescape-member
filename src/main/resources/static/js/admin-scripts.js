...
function updateUIBasedOnLogin() {
  ...
  .then(data => {
    ...
    document.querySelector(
        '.nav-item a[href="/login"]').parentElement.style.display = 'none'; // 로그인 버튼 숨김
  })
  .catch(error => {
    ...
    document.querySelector(
        '.nav-item a[href="/login"]').parentElement.style.display = 'block'; // 로그인 버튼 표시
  });
}
...
document.getElementById("navbarDropdown").addEventListener('click',
    function (e) {
      e.preventDefault();
      const dropdownMenu = e.target.closest('.nav-item.dropdown').querySelector(
          '.dropdown-menu');
