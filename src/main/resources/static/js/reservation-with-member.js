...
const FILTER_API_ENDPOINT = '/reservations/filter'
const timesOptions = [];
const themesOptions = [];
...
function render(data) {
    ...
    data.forEach(item => {
        ...
        row.insertCell(1).textContent = item.member.name; // 유저 name
        row.insertCell(2).textContent = item.theme.name;      // 테마 name
        row.insertCell(3).textContent = item.date;            // date
        row.insertCell(4).textContent = item.reservationTime.startAt;    // 예약 시간 startAt
...
function applyFilter(event) {
    ...
    fetch(`${FILTER_API_ENDPOINT}?themeId=${themeId}&memberId=${memberId}&dateFrom=${dateFrom}&dateTo=${dateTo}`, { // 예약 검색 API 호출
        method: 'GET',
        headers: {
