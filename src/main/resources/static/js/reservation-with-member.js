...
function applyFilter(event) {
  ...
  const themeIdParam = themeId === "" ? themeId : `&themeId=${themeId}`
  const memberIdParam = memberId === "" ? memberId : `&memberId=${memberId}`
  const dateFromParam = dateFrom === "" ? dateFrom : `&fromDate=${dateFrom}`
  const dateToParam = dateTo === "" ? dateTo : `&endDate=${dateTo}`

  fetch(`/reservations?${themeIdParam + memberIdParam + dateFromParam + dateToParam}`, { // 예약 검색 API 호출
    method: 'GET',
    headers: {
