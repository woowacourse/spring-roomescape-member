...
const RESERVATION_SEARCH_API_ENDPOINT = '/admin/reservations';
...
function render(data) {
  ...
  data.forEach(item => {
    ...
    row.insertCell(1).textContent = item.memberInfo.name;     // 사용자 name
...
function applyFilter(event) {
  ...
  fetch(`${RESERVATION_SEARCH_API_ENDPOINT}?themeId=${themeId}&memberId=${memberId}&from=${from}&to=${to}`, { // 예약 검색 API 호출
    method: 'GET',
    headers: {
