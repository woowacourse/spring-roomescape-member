...
class ReservationCookieRequestTest {
    ...
    void throw_exception_when_null_date_input(String date) {
        assertThatThrownBy(() -> new ReservationCookieRequest(date, validThemeId, validTimeId))
    ...
    void throw_exception_when_null_time_id_input() {
        assertThatThrownBy(() -> new ReservationCookieRequest(validDate, validThemeId, null))
                .isInstanceOf(IllegalArgumentException.class)
    ...
    void throw_exception_when_not_natural_time_id_input(Long timeId) {
        assertThatThrownBy(() -> new ReservationCookieRequest(validDate, validThemeId, timeId))
                .isInstanceOf(IllegalArgumentException.class)
    ...
    void throw_exception_when_null_theme_id_input() {
        assertThatThrownBy(() -> new ReservationCookieRequest(validDate, null, validTimeId))
                .isInstanceOf(IllegalArgumentException.class)
    ...
    void throw_exception_when_not_natural_theme_id_input(Long themeId) {
        assertThatThrownBy(() -> new ReservationCookieRequest(validDate, themeId, validTimeId))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessage("아이디는 자연수여야 합니다.");
    }
    ...
    void create_success() {
        assertThatNoException()
                .isThrownBy(() -> new ReservationCookieRequest(validDate, validThemeId, validTimeId));
    }
}
