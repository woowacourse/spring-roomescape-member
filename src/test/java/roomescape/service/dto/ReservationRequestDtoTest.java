...
class ReservationRequestDtoTest {
    ...
    void throw_exception_when_null_name_input(String name) {
        assertThatThrownBy(() -> new ReservationRequestDto(name, validDate, validTimeId))
    ...
    void throw_exception_when_null_date_input(String date) {
        assertThatThrownBy(() -> new ReservationRequestDto(validName, date, validTimeId))
                .isInstanceOf(IllegalArgumentException.class)
    ...
    void throw_exception_when_null_time_id_input() {
        assertThatThrownBy(() -> new ReservationRequestDto(validName, validDate, null))
                .isInstanceOf(IllegalArgumentException.class)
    ...
    void throw_exception_when_not_natural_time_id_input(Long timeId) {
        assertThatThrownBy(() -> new ReservationRequestDto(validName, validDate, timeId))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessage("시간 아이디는 자연수여야 합니다.");
    }
    ...
    void create_success() {
        assertThatNoException()
                .isThrownBy(() -> new ReservationRequestDto(validName, validDate, validTimeId));
    }
}
