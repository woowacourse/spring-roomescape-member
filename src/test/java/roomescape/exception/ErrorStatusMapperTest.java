...
class ErrorStatusMapperTest {
    ...
    void 예약_검증_예외를_bad_request로_매핑한다() {
        ...
        assertThat(errorStatusMapper.map(ErrorCode.RESERVATION_ALREADY_HAS_ID)).isEqualTo(HttpStatus.CONFLICT);
        assertThat(errorStatusMapper.map(ErrorCode.RESERVATION_ALREADY_EXISTS)).isEqualTo(HttpStatus.CONFLICT);
        assertThat(errorStatusMapper.map(ErrorCode.RESERVATION_TIME_ALREADY_HAS_ID)).isEqualTo(HttpStatus.CONFLICT);
        assertThat(errorStatusMapper.map(ErrorCode.RESERVATION_TIME_ALREADY_EXISTS)).isEqualTo(HttpStatus.CONFLICT);
        ...
        assertThat(errorStatusMapper.map(ErrorCode.THEME_ALREADY_HAS_ID)).isEqualTo(HttpStatus.CONFLICT);
