...
public class ReservationServiceTest {
    ...
    void 모든_예약을_조회한다() {
        ...
        Reservation reservation = new Reservation(1L, "홍길동", 예약날짜_오늘.getDate(), reservationTime, theme);
        ...
        softly.assertThat(response.theme().name()).isEqualTo(theme.getName());
        softly.assertThat(response.theme().description()).isEqualTo(theme.getDescription());
        softly.assertThat(response.theme().thumbnail()).isEqualTo(theme.getThumbnail());
        softly.assertAll();
    }

    @Test
    void 예약을_생성할_수_있다() {
        ReservationCreateRequest request = new ReservationCreateRequest("홍길동",  예약날짜_오늘.getDate(), 1L, 1L);
        ReservationTime reservationTime = new ReservationTime(1L, time);
        Theme theme = new Theme(1L, "공포", "무섭다", "thumb.jpg");
        ...
        when(reservationRepository.save(any(), any(), any())).thenReturn(
                new Reservation(1L, "홍길동",  예약날짜_오늘.getDate(), reservationTime, theme)
        );
    ...
    void 예약시간이_없으면_예외가_발생한다() {
        ReservationCreateRequest request = new ReservationCreateRequest("홍길동",  예약날짜_오늘.getDate(), 999L, 1L);
