...
public class ReservationCreateRequestValidationTest {
    ...
    void invalidDate() {
        ReservationCreateRequest request = new ReservationCreateRequest(null, 1L, 1L);
        Set<ConstraintViolation<ReservationCreateRequest>> violations = validator.validate(request);
        for (ConstraintViolation<ReservationCreateRequest> violation : violations) {
    ...
    void invalidTime() {
        ReservationCreateRequest request = new ReservationCreateRequest(LocalDate.now(), null, 1L);
        Set<ConstraintViolation<ReservationCreateRequest>> violations = validator.validate(request);
        for (ConstraintViolation<ReservationCreateRequest> violation : violations) {
    ...
    void invalidTheme() {
        ReservationCreateRequest request = new ReservationCreateRequest(LocalDate.now(), 1L, null);
        Set<ConstraintViolation<ReservationCreateRequest>> violations = validator.validate(request);
        for (ConstraintViolation<ReservationCreateRequest> violation : violations) {
