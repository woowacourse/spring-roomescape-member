...
class UserReservationControllerTest {
    ...
    void 예약_취소_테스트() throws Exception {
        // when & then
        mockMvc.perform(delete("/reservations/user/{id}", 1L))
    ...
    void 존재하지_않는_예약_취소_404_반환_테스트() throws Exception {
        ...
        // when & then
        mockMvc.perform(delete("/reservations/user/{id}", 999L))
    ...
    void 예약_변경_테스트() throws Exception {
        ...
        // when & then
        mockMvc.perform(put("/reservations/user/{id}", 1L)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(requestBody))
                .andExpect(status().isOk())
    ...
    void 존재하지_않는_예약_변경_404_반환_테스트() throws Exception {
        ...
        // when & then
        mockMvc.perform(put("/reservations/user/{id}", 999L)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(requestBody))
                .andExpect(status().isNotFound());
    }
