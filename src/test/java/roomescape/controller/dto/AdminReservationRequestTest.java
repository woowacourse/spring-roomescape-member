...
class AdminReservationRequestTest {
    ...
    @DisplayName("회원 아이디가 입력되지 않으면 예외가 발생한다.")
    @Test
    void throw_exception_when_null_member_id_input() {
        AdminReservationRequest requestDto = new AdminReservationRequest(null, validThemeId, validDate, validTimeId);

        Set<ConstraintViolation<AdminReservationRequest>> violations = validator.validate(requestDto);

        assertThat(violations).extracting("message").
                containsOnly("회원 아이디는 반드시 입력되어야 합니다.");
    }

    @DisplayName("회원 아이디가 자연수가 아니면 예외가 발생한다.")
    @ParameterizedTest
    @ValueSource(longs = {0, -1})
    void throw_exception_when_not_natural_member_id_input(Long memberId) {
        AdminReservationRequest requestDto = new AdminReservationRequest(memberId, validThemeId, validDate,
                validTimeId);

        Set<ConstraintViolation<AdminReservationRequest>> violations = validator.validate(requestDto);

        assertThat(violations).extracting("message").
                containsOnly("회원 아이디는 자연수여야 합니다. " + memberId + "은 사용할 수 없습니다.");
    }

    @DisplayName("테마 아이디가 입력되지 않으면 예외가 발생한다.")
    @Test
    void throw_exception_when_null_theme_id_input() {
        AdminReservationRequest requestDto = new AdminReservationRequest(validMemberId, null, validDate, validTimeId);
    ...
    void throw_exception_when_not_natural_theme_id_input(Long themeId) {
        AdminReservationRequest requestDto = new AdminReservationRequest(validMemberId, themeId, validDate,
                validTimeId);
    ...
    void throw_exception_when_invalid_date_format_input(String date) {
        AdminReservationRequest requestDto = new AdminReservationRequest(validMemberId, validThemeId, date,
                validTimeId);
    ...
    void throw_exception_when_null_time_id_input() {
        AdminReservationRequest requestDto = new AdminReservationRequest(validMemberId, validThemeId, validDate, null);
    ...
    void throw_exception_when_not_natural_time_id_input(Long timeId) {
        AdminReservationRequest requestDto = new AdminReservationRequest(validMemberId, validThemeId, validDate,
                timeId);
