...
class ReservationCreateValidatorTest {
    private final ReservationRequest reservationRequest = new ReservationRequest("reservationName", LocalDate.parse("2099-11-22"), 1L, 2L);
    ...
    void given_when_new_then_doesNotException() {
        //when, then
        assertThatCode(() -> new ReservationCreateValidator(reservationRequest, timeSlot, theme)).doesNotThrowAnyException();
    }
    ...
    void given_pastReservationRequest_when_new_then_thrownReservationFailException() {
        //given
        ReservationRequest pastReservationRequest = new ReservationRequest("reservationName", LocalDate.parse("1999-11-22"), 1L, 2L);
        TimeSlot timeSlot = new TimeSlot(pastReservationRequest.timeId(), LocalTime.parse("10:00"));
        Theme theme = new Theme(pastReservationRequest.themeId(), "themeName", "description", "thumbnail");
        //when, then
        assertThatThrownBy(() -> new ReservationCreateValidator(pastReservationRequest, timeSlot, theme)).isInstanceOf(ReservationFailException.class);
    }
