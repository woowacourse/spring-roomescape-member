...
public class ReservationTimeAcceptanceTest {
    ...
    void reservationTimeApiSuccessTest() {
        ...
        ReservationTimeSteps.checkAvailableReservation(futureDateTime.toLocalDate().toString(), 1L, true);

        // 5. 예약 추가
        ReservationSteps.createReservation("예약자", futureDateTime.toLocalDate().toString(), 1L, 1L);

        // 6. 예약 추가 후 해당 날짜, 테마의 시간 예약 가능 상태 변경 조회
        ReservationTimeSteps.checkAvailableReservation(futureDateTime.toLocalDate().toString(), 1L, false);
