...
VALUES
    ...
    (1, '루드비코', DATEADD('DAY', -7, CURRENT_DATE), 1, 1),
    (2, '어셔', DATEADD('DAY', -6, CURRENT_DATE), 2, 1),
    (3, '브라운', DATEADD('DAY', -5, CURRENT_DATE), 3, 1),
    (4, '제이슨', DATEADD('DAY', -4, CURRENT_DATE), 4, 1),
    (5, '루드비코', DATEADD('DAY', -3, CURRENT_DATE), 1, 1),

    -- 2위: 박물관 침입 (4건) - 최근 4~7일 전 데이터
    (6, '코코', DATEADD('DAY', -7, CURRENT_DATE), 1, 2),
    (7, '공구', DATEADD('DAY', -6, CURRENT_DATE), 2, 2),
    (8, '구구', DATEADD('DAY', -5, CURRENT_DATE), 3, 2),
    (9, '주디', DATEADD('DAY', -4, CURRENT_DATE), 4, 2),

    -- 3위: 셜록의 서재 (3건)
    (10, '참가자1', DATEADD('DAY', -3, CURRENT_DATE), 1, 3),
    (11, '참가자2', DATEADD('DAY', -2, CURRENT_DATE), 2, 3),
    (12, '참가자3', DATEADD('DAY', -2, CURRENT_DATE), 3, 3),

    -- 4위: 서부의 무법자 (2건)
    (13, '참가자4', DATEADD('DAY', -7, CURRENT_DATE), 1, 4),
    (14, '참가자5', DATEADD('DAY', -6, CURRENT_DATE), 2, 4),

    -- 5위: 심해의 비밀 (2건)
    (15, '참가자6', DATEADD('DAY', -5, CURRENT_DATE), 1, 5),
    (16, '참가자7', DATEADD('DAY', -4, CURRENT_DATE), 2, 5),

    -- 6위: 우주선 탈출 (2건)
    (17, '참가자8', DATEADD('DAY', -3, CURRENT_DATE), 1, 6),
    (18, '참가자9', DATEADD('DAY', -2, CURRENT_DATE), 2, 6),

    -- 7위: 감옥 탈출 (1건)
    (19, '참가자10', DATEADD('DAY', -2, CURRENT_DATE), 1, 7),

    -- 8위: 마법사의 방 (1건)
    (20, '참가자11', DATEADD('DAY', -7, CURRENT_DATE), 2, 8),

    -- 예외 케이스: 집계 범위 밖 (너무 과거이거나 오늘/미래 데이터)
    -- 15일 전
    (21, '나그네', DATEADD('DAY', -15, CURRENT_DATE), 1, 11),
    -- 오늘
    (22, '방문객', CURRENT_DATE, 2, 11),
    -- 미래
    (23, '루드비코', DATEADD('DAY', 1, CURRENT_DATE), 1, 1),
    (24, '루드비코', DATEADD('DAY', 2, CURRENT_DATE), 1, 1);