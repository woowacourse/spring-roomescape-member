...
|----|-----------------------------------------|---------------------------|-----------------------|
| R1 | 요청 처리 성공                                | 201 Created               | 없음                    |
| R2 | 요청 본문 JSON 형식 또는 필드 자료형이 올바르지 않은 경우     | 400 Bad Request           | 요청값이 올바르지 않습니다.       |
| R3 | DTO 필드 제약 조건을 만족하지 못한 경우                | 400 Bad Request           | 요청 값이 올바르지 않습니다.      |
| R4 | 참조 자원이 존재하지 않는 경우                       | 404 Not Found             | 조회할 자원이 존재하지 않습니다.    |
| R5 | date, timeId, themeId에 해당하는 예약이 존재하는 경우 | 409 Conflict              | 이미 예약된 날짜, 시간, 테마입니다. |
| R6 | 서버 오류인 경우                               | 500 Internal Server Error | 예상치 못한 오류가 발생했습니다.    |

### 필드별 에러 메시지

| ID   | 상황                   | 문제 필드명  | 필드 에러 메시지               |
|------|----------------------|---------|-------------------------|
| R3-1 | name이 공백이거나 null인 경우 | name    | 예약자명은 필수입니다.            |
| R3-2 | name의 길이가 20 초과인 경우  | name    | 예약자명의 길이는 1이상 20이하 입니다. |
| R3-3 | date가 null인 경우       | date    | 예약 날짜는 필수입니다.           |
| R3-4 | date가 현재 날짜보다 과거인 경우 | date    | 예약 날짜가 현재보다 과거입니다.      |
| R3-5 | timeId가 null인 경우     | timeId  | timeId는 필수입니다.          |
| R3-6 | timeId가 양수가 아닌 경우    | timeId  | timeId의 값이 유효하지 않습니다.   |
| R3-7 | themeId가 null인 경우    | themeId | themeId는 필수입니다.         |
| R3-8 | themeId가 양수가 아닌 경우   | themeId | themeId의 값이 유효하지 않습니다.  |
| R4-1 | timeId가 존재하지 않는 경우   | timeId  | 존재 하지 않는 시간대입니다.        |
| R4-2 | themeId가 존재하지 않는 경우  | themeId | 존재 하지 않는 테마입니다.         |
