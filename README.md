...
8. 삭제되지 않은 예약 중 변경하려는 날짜, 시간, 테마의 예약이 이미 있으면 변경할 수 없습니다.
9. 자기 자신 예약은 중복 예약 검사에서 제외합니다.
10. 삭제된 예약은 중복 예약 검사에서 제외합니다.
```
...
| RP6  | 예약 ID가 존재하지 않는 경우                       | 404 Not Found             | 예약을 찾을 수 없습니다.        |
| RP7  | 조회한 예약의 예약자 이름과 요청 예약자 이름이 다른 경우       | 403 Forbidden             | 예약을 변경할 권한이 없습니다.     |
| RP8  | 참조 자원이 존재하지 않는 경우                       | 404 Not Found             | 수정할 자원이 존재하지 않습니다.    |
| RP9  | 이미 지난 예약을 변경하려는 경우                      | 409 Conflict              | 지난 예약은 변경할 수 없습니다.    |
| RP10 | date, timeId, themeId에 해당하는 예약이 존재하는 경우 | 409 Conflict              | 이미 예약된 날짜, 시간, 테마입니다. |
| RP11 | 서버 오류인 경우                               | 500 Internal Server Error | 예상치 못한 오류가 발생했습니다.    |
