...
### 이름으로 예약 조회 - GET /reservations?name={name}

**요청**
- Method: `GET`
- URL: `/reservations?name={name}`
- Query Parameter: `name` (예약자 이름)
...
**예외**
- 404: 해당 이름의 예약이 존재하지 않음

### 예약 변경 - PATCH /reservations/{id}
...
```
> `name`, `date` 생략 시 기존 값 유지. `timeId`, `themeId` 필수.
...
**예외**
- 400: 이름 20자 초과 / 잘못된 날짜 형식 / 이미 지난 날짜·시간으로 변경
- 404: 존재하지 않는 예약 id / `timeId` / `themeId`
- 409: 변경하려는 날짜·시간·테마 슬롯이 이미 예약됨

### 예약 취소 - DELETE /reservations/{id}?name={name}

**요청**
- Method: `DELETE`
- URL: `/reservations/{id}?name={name}`
- Query Parameter: `name` (예약자 이름, 본인 확인용)
...
**예외**
- 400: 이미 지난 예약 취소 불가
- 404: 존재하지 않는 id / 해당 id의 예약자 이름과 불일치
...
- 예약자 이름을 입력하면 해당 이름의 예약을 조회한다.
- 존재하지 않는 이름 → "존재하지 않는 예약입니다" 안내.

## U7. 내 예약 취소

- 예약 id와 예약자 이름을 함께 입력해 취소한다.
- 이름이 일치하지 않으면 취소할 수 없다.
- **이미 지난 예약은 취소할 수 없다.**
- 성공 시 해당 예약이 삭제된다.

## U8. 내 예약 변경

- 예약 id를 지정해 날짜·시간·테마(·이름)를 변경한다.
- `timeId`, `themeId`는 필수. `name`, `date`는 생략 시 기존 값 유지.
- **이미 지난 날짜·시간으로는 변경할 수 없다.**
- **변경하려는 날짜·시간·테마 슬롯이 이미 예약된 경우 변경할 수 없다.**
...
| 규칙 | 위반 시 응답 |
|------|--------------|
| 이름이 예약자 이름과 일치해야 함 | 404 "존재하지 않는 예약입니다." |
| 이미 지난 예약은 취소 불가 | 400 "이미 지난 예약입니다." |

## R5. 예약 변경 시 (사용자)

| 규칙 | 위반 시 응답 |
|------|--------------|
| `timeId`, `themeId` 필수 | 400 항목별 안내 메시지 |
| 이름 최대 20자 | 400 "이름은 20자 이하여야 합니다" |
| 이미 지난 날짜·시간으로 변경 불가 | 400 "이미 지난 시각으로는 예약할 수 없습니다." |
| 예약 가능 범위 초과(현재 기준 14일 이후) 변경 불가 | 400 "예약은 현재로부터 최대 14일 이내만 가능합니다." |
| 변경하려는 날짜·시간·테마 슬롯이 이미 예약됨 | 409 "이미 예약된 시간입니다." |
