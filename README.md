...
- [x] 과거 날짜,시간 예약 거부 (ReservationService에 Clock 주입)
- [x] 같은 날짜+시간+테마 중복 예약 거부 (Repository에 existsBy... 추가)
- [x] 예약이 존재하는 시간 삭제 거부
- [x] 도메인 입력 검증 보강 (차후...마지막에 다시 고려)

Phase 2 - 예외 계층 & 전역 예외 처리

- [x] 커스텀 예외 추가 (BusinessRuleViolationException, ResourceNotFoundException)
- [x] 에러 응답 DTO 추가 (ErrorResponse)
- [x] @RestControllerAdvice로 전역 핸들러 작성
- [x] 스프링 기본 예외(MethodArgumentTypeMismatchException.... 등등등) 핸들링
- [x] 500이 사용자에게 노출되지 않도록 fallback 핸들러 추가

Phase 3 - 내 예약 조회/변경/취소

- [x] GET /user/reservations?name= (내 예약 목록)
- [x] PATCH /user/reservations/{id} (날짜/시간 변경, 본인 검증)
- [x] DELETE /user/reservations/{id} (취소, 본인 검증)
- [x] 변경/취소의 거부 케이스 적용 (과거 예약, 시간 충돌, 남의 예약 등등)

Phase 4 - 화면 & 테스트

- [x] 내 예약 페이지(my-reservations.html) 추가
- [x] 에러 응답이 화면에 의미 있게 표시되도록 처리
- [x] 각 거부 케이스 요구사항 테스트 추가
- [x] 변경/취소 정상 흐름 요구사항 테스트 추가

### 사용자 API (`/user/...`) - 사이클 2 추가
