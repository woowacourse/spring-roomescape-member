...
### 예약 수정 - PATCH /admin/reservations/{id}
...
**예외**
- 400: 필수 값(`startAt`) 누락 / 잘못된 시간 형식 (예: `"25:00"`, `"abc"`) / 영업 시간(10:00~22:00) 범위 초과
- 409: 동일한 시간 중복 등록
...
**예외**
- 400: 이름 2자 미만 또는 15자 초과 / 유효하지 않은 URL 형식
- 409: 동일한 이름 테마 중복 등록
...
- Query Parameter:
  - `limit`: 조회할 테마 수 (기본값 10, 최대 15)
  - `days`: 집계 기준 일수 (기본값 7, 최대 10) — 오늘 포함 `days`일 간의 예약을 기준으로 집계
...
- 집계 기준: 오늘 포함 **오늘부터 6일 전까지의 게임 날짜**를 가진 예약을 집계한다.
  - 예: 오늘이 5월 8일이면, 게임 날짜가 5월 2일 ~ 5월 8일인 예약을 집계.
...
- 예약자는 자신의 예약을 취소할 수 있다.
- 예약 일시가 이미 지난 경우에는 취소할 수 없다.
...
- 시작 시각을 입력해 등록한다.
- 영업 시간(10:00~22:00) 범위 내의 시각만 등록 가능하다.
...
| 규칙 | 위반 시 응답 |
|------|--------------|
| 예약 일시가 이미 지난 경우 취소 불가 | "지난 예약은 취소 불가능합니다. 관리자에게 문의하세요" |
...
| 시간: 10:00~22:00 범위 외 불가 | "영업 시간은 10시부터 22시 사이입니다." |
| 테마: 같은 이름 중복 불가 | "이미 존재하는 테마 이름입니다." |
| 테마 이름: 2자 미만 또는 15자 초과 불가 | "이름은 2~15글자만 가능합니다." |
