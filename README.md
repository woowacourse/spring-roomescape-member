...
| RC2 | 요청 본문 JSON 형식 또는 필드 자료형이 올바르지 않은 경우     | 400 Bad Request           | 요청값이 올바르지 않습니다.       |
...
| RD2 | 예약 ID의 자료형이 올바르지 않은 경우 | 400 Bad Request           | 요청값이 올바르지 않습니다.    |
| RD3 | 예약 ID가 존재하지 않는 경우      | 404 Not Found             | 예약을 찾을 수 없습니다.     |
| RD4 | 서버 오류인 경우              | 500 Internal Server Error | 예상치 못한 오류가 발생했습니다. |

### 파라미터별 에러 메시지

| ID    | 상황               | 문제 파라미터명 | 파라미터 에러 메시지      |
|-------|------------------|----------|------------------|
| RD2-1 | id의 값이 양수가 아닌 경우 | id       | id의 값은 양수여야 합니다. |
...
| TA2 | Query Parameter가 올바르지 않은 경우  | 400 Bad Request           | 요청값이 올바르지 않습니다.    |
| TA3 | themeId에 해당하는 테마가 존재하지 않는 경우 | 404 Not Found             | 조회할 자원이 존재하지 않습니다. |
| TA4 | 서버 오류인 경우                    | 500 Internal Server Error | 예상치 못한 오류가 발생했습니다. |

### 파라미터별 에러 메시지

| ID    | 상황                  | 문제 파라미터명 | 파라미터 에러 메시지           |
|-------|---------------------|----------|-----------------------|
| TA2-1 | themeId가 양수가 아닌 경우  | themeId  | themeId의 값은 양수여야 합니다. |
| TA3-1 | themeId가 존재하지 않는 경우 | themeId  | 존재 하지 않는 테마입니다.       |
...
| TC2 | 요청 본문 JSON 형식 또는 필드 자료형이 올바르지 않은 경우 | 400 Bad Request           | 요청값이 올바르지 않습니다.    |
...
| TD2 | 예약 시간 ID의 자료형이 올바르지 않은 경우 | 400 Bad Request           | 요청값이 올바르지 않습니다.    |
| TD3 | 예약 시간 ID가 존재하지 않는 경우      | 404 Not Found             | 예약 시간을 찾을 수 없습니다.  |
| TD4 | 서버 오류인 경우                 | 500 Internal Server Error | 예상치 못한 오류가 발생했습니다. |

### 파라미터별 에러 메시지

| ID    | 상황               | 문제 파라미터명 | 파라미터 에러 메시지      |
|-------|------------------|----------|------------------|
| TD2-1 | id의 값이 양수가 아닌 경우 | id       | id의 값은 양수여야 합니다. |
...
- 오늘 날짜는 집계 기간에서 제외합니다.
- 오늘을 제외한 직전 7일 동안 생성된 예약 수를 기준으로 테마를 조회합니다.
- 예약 수가 많은 순서로 정렬합니다.
- 예약 수가 같으면 테마 ID가 작은 순서로 정렬합니다.
- 최대 10개의 테마를 반환합니다.
- 조회 가능한 인기 테마가 없으면 빈 목록을 반환합니다.
- `deleted_at IS NULL`인 테마만 조회 결과에 포함합니다.
- `deleted_at IS NULL`인 예약만 인기순 집계에 반영합니다.
- `deleted_at IS NULL`인 예약 시간에 연결된 예약만 인기순 집계에 반영합니다.
...
| HC2 | 요청 본문 JSON 형식 또는 필드 자료형이 올바르지 않은 경우 | 400 Bad Request           | 요청값이 올바르지 않습니다.    |
...
| HD2 | 테마 ID의 자료형이 올바르지 않은 경우 | 400 Bad Request           | 요청값이 올바르지 않습니다.    |
| HD3 | 테마 ID가 존재하지 않는 경우   | 404 Not Found             | 테마를 찾을 수 없습니다.     |
| HD4 | 서버 오류인 경우           | 500 Internal Server Error | 예상치 못한 오류가 발생했습니다. |

### 파라미터별 에러 메시지

| ID    | 상황               | 문제 파라미터명 | 파라미터 에러 메시지      |
|-------|------------------|----------|------------------|
| HD2-1 | id의 값이 양수가 아닌 경우 | id       | id의 값은 양수여야 합니다. |
