# 사이클 1 기능 목록

## 1단계 :

- [x] 예약에 테마 정보를 포함하도록 한다.
- [x] 테마는 이름, 설명, 썸네일 이미지 URL을 가진다.
- [x] 스케줄 시작 시간은 10시이고, 종료 시간은 20시이다.
- [x] 각 테마의 소요 시간은 2시간이다.
- [x] 관리자가 테마를 추가, 삭제할 수 있다.

## 2단계 :

- [x] 사용자가 날짜와 테마를 선택하면 예약 가능한 시간 목록이 표시된다.
- [x] 사용자가 예약 가능한 시간을 선택하여 본인의 이름으로 예약한다.
- [x] 같은 날짜, 시간이라도 테마가 다르면 각각 예약 가능하다.

## 3단계 :

- [x] 최근 1주 동안 예약이 많았던 테마 상위 10개를 조회한다.
  - 예: 오늘이 5월 8일이면, 게임 날짜가 5월 1일 ~ 5월 7일인 예약을 집계해 인기 순서대로 10개를 응답한다.

# 사이클 2 기능 목록

## 1단계 : 

다음 정책을 만족하지 않는 요청을 거부한다
- [x] 지나간 날짜·시간에 대한 예약 생성은 불가능하다. (사용자 요청)
- [x] 같은 날짜+시간+테마에 이미 예약이 있으면 중복 예약을 거부한다. (사용자 요청)
- [x] 예약이 존재하는 시간을 삭제할 수 없다. (관리자 권한)
- [x] 유효하지 않은 입력값(빈 이름, 잘못된 날짜 형식 등)을 거부한다. (사용자 요청)


## 2단계 : 

- [x] 의도된 에러 응답을 반환한다.
- [x] 500(서버 에러)이 사용자에게 노출되지 않도록 한다.

## 3단계 : 

- [ ] 사용자가 자신의 이름으로 본인의 예약 목록을 조회할 수 있다.
- [ ] 사용자가 본인의 예약을 취소할 수 있다.
- [ ] 사용자가 본인의 예약의 날짜·시간을 변경할 수 있다.
---

# API 명세서

## 관리자 API 명세서

### 1. 테마 추가
| 항목 | 상세 내용                                                                                         |
| :--- |:----------------------------------------------------------------------------------------------|
| **URL** | `/admin/themes`                                                                               |
| **HTTP Method** | `POST`                                                                                        |
| **Request (Body)** | `{ "name": "테마명", "requiredTime" : "소요시간" }`                                                  |
| **Response** | `{ "id": 1, "name": "테마명", "description":"설명", "imageUrl":"이미지경로", "requiredTime" : "소요시간" }` |
| **StatusCode** | `201 Created`                                                                                 |

### 2. 테마 삭제
| 항목 | 상세 내용 |
| :--- | :--- |
| **URL** | `/admin/themes/{id}` |
| **HTTP Method** | `DELETE` |
| **Request (Path)** | `id` (테마 ID) |
| **Response** | 없음 |
| **StatusCode** | `204 No Content` |

### 3. 전체 예약 조회
| 항목 | 상세 내용                                                                                                                                                                          |
| :--- |:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **URL** | `/admin/reservations`                                                                                                                                                          |
| **HTTP Method** | `GET`                                                                                                                                                                          |
| **Request** | 없음                                                                                                                                                                             |
| **Response** | `[ { "reservationId": 1, "userId": 1, "userName": "루크", "themeId": 1, "themeName": "테마명", "scheduleId": 1, startAt": "2026-05-10 15:00", "endAt": "2026-05-10 17:00" }, ... ]` |
| **StatusCode** | `200 OK`                                                                                                                                                                       |

### 4. 예약 삭제
| 항목 | 상세 내용 |
| :--- | :--- |
| **URL** | `/admin/reservations/{id}` |
| **HTTP Method** | `DELETE` |
| **Request (Path)** | `id` (예약 ID) |
| **Response** | 없음 |
| **StatusCode** | `204 No Content` |

## 사용자 API 명세서

### 1. 테마 목록 조회

| 항목              | 상세 내용                                                                                    |
|:----------------|:-----------------------------------------------------------------------------------------|
| **URL**         | `/themes`                                                                                |
| **HTTP Method** | `GET`                                                                                    |
| **Request**     | 없음                                                                                       |
| **Response**    | `[ { "id": 1, "name": "테마명", "description": "공포", "image_url": "https://techcourse.woowahan.com/" }, ... ]` |
| **StatusCode**  | `200 OK`                                                                                 |

### 2. 예약된 스케줄 조회

| 항목 | 상세 내용                                                                                                                        |
| :--- |:-----------------------------------------------------------------------------------------------------------------------------|
| **URL** | `/schedules?date={date}&themeId={themeId}`                                                                                   |
| **HTTP Method** | `GET`                                                                                                                        |
| **Request (Query)** | `date`, `themeId`                                                                                                            |
| **Response** | `[ { "id": 1, "themeName": "테마명", "startAt": "2026-05-10 10:00", "endAt": "2026-05-10 12:00" }, ... ]` |
| **StatusCode**  | `200 OK`                                                                                                                     |

### 3. 예약 생성

| 항목 | 상세 내용                                           |
| :--- |:------------------------------------------------|
| **URL** | `/reservations`                                 |
| **HTTP Method** | `POST`                                          |
| **Request (Body)** | `{ "startAt":"날짜시간", "themeId":1, "userId":1 }` |
| **Response** | `{ "id": 101 }`                                 |
| **StatusCode**  | `201 Created`                                   |

### 4. 인기 테마 통계 조회

| 항목 | 상세 내용                                                                                       |
| :--- |:--------------------------------------------------------------------------------------------|
| **URL** | `/themes?sort=reservations&limit=10&days=7`                                                 |
| **HTTP Method** | `GET`                                                                                       |
| **Request (Query)** | `sort`: 정렬 기준 (예: `reservations`) <br> `limit`: 조회 개수 (예: `10`) <br> `days`: 통계 기간 (예: `7`) |
| **Response** | `[ { "themeName": "테마명", "reservationCount": 25 }, ... ]`                                   |
| **StatusCode**  | `200 OK`                                                                                    | 

### 5. 본인 예약 목록 조회
| 항목 | 상세 내용                                                                                                                                                                       |
| :--- |:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **URL** | `/reservations/my`                                                                                                                                                          |
| **HTTP Method** | `GET`                                                                                                                                                                       |
| **Request** | `-`                                                                                                                                                                           |
| **Response** | `[ {"reservationId": 1, "userId": 1, "userName": "루크", "themeId": 1, "themeName": "테마명", "scheduleId": 1, startAt": "2026-05-10 15:00", "endAt": "2026-05-10 17:00"}, ...]` |
| **StatusCode**  | `200 ok`                                                                                                                                                                    |### 5. 본인 예약 목록 조회

### 6. 본인 예약 취소
| 항목 | 상세 내용                |
| :--- |:---------------------|
| **URL** | `/reservations/{id}` |
| **HTTP Method** | `DELETE`             |
| **Request** | `-`                    |
| **Response** | `-`                  |
| **StatusCode**  | `204 no content`     |

### 7. 본인 예약 날짜·시간 변경
| 항목                 | 상세 내용                                       |
|:-------------------|:--------------------------------------------|
| **URL**            | `/reservations/{id}`                        |
| **HTTP Method**    | `PATCH`                                     |
| **Request (Body)** | `{ "scheduleId": 2 }`                       |
| **Response**       | `{ "id":1L, "userId":1L, "scheduleId":2L }` |
| **StatusCode**     | `200 ok`                                    |