...
| `errors` | 상세 에러 리스트 | `400 Bad Request` 시에만 포함 |
...
| **400** | 타입 불일치 | 경로 변수나 파라미터 타입이 틀릴 시 | "요청 데이터의 타입이 올바르지 않습니다." |
...
주로 `@Valid` 어노테이션에 의한 검증 실패 시 반환됩니다.
...
숫자가 필요한 ID 자리에 문자열이 오는 등 타입이 맞지 않을 때 반환됩니다.

```json
{
  "status": 400,
  "title": "타입 불일치",
  "detail": "요청 데이터의 타입이 올바르지 않습니다.",
  "instance": "/admin/reservations/abc",
  "errors": [
    {
      "field": "id",
      "value": "abc",
      "message": "Long 타입이어야 합니다."
    }
  ]
}
```
