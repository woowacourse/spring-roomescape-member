...
- [ ] 테마 내역은 이름, 설명, 썸네일 이미지 URL을 갖는다.
- [ ] 테마에 상관없이 오픈 시간과 종료 시간이 같다.
- [ ] 모든 테마의 소요시간 1시간으로 통일한다.
- [ ] 예약 내역에는 테마 정보가 포함된다.
...
- [ ] 사용자가 날짜와 테마를 선택하면 예약 가능한 시간 목록이 표시된다.
    - [ ] 예약 가능한 시간이란 이전 예약 리스트에 동일한 날짜 + 테마의 예약이 없는 시간이다.
    - [ ] 테마 예약이 더 이상 불가능해도 선택할 수 있다.
- [ ] 예약할 때 이름을 입력받아야 하고, 예약 기록에 이름을 포함한다.
- [ ] 테마가 다르다면 동일한 시간에 동일한 이름을 가진 예약을 할 수 있다.

### 인기 테마 조회
- [ ] 최근 1주 동안 예약이 많았던 테마 상위 10개를 조회할 수 있다.
- [ ] 조회 기준은 오늘을 제외한 이전 7일을 기준으로 한다.

### 구현 관련
- [ ] `data.sql`에 기본 시간을 추가해둔다.
...
| 테마 목록 조회 | `GET /themes` | - | `orderBy`, `count`, `period` | `200 OK` <br> `[{themeId, name, description, url}, ...]` |
