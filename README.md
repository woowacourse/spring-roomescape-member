...
* [ ] 지나간 날짜 시간에 대한 예약 생성은 거부한다.
* [ ] 같은 날짜/시간/테마에 이미 예약이 있으면 중복 예약을 거부한다.
* 유효하지 않은 입력값은 거부
    * [ ] 이름이 비어 있으면 거부
    * [ ] 날짜 형식이 올바르지 않으면 거부
    * [ ] 시간 ID, 테마 ID 존재하지 않으면 거부

### 예약 시간/ 테마 정책 검증

* [ ] 예약이 존재하는 시간, 테마를 삭제할 수 없다.

### 에러 응답 설계

* [ ] 예외 처리를 일관되게 처리하는 전역 예외 처리기 구현.
* [ ] 서비스 정책 위반, 유효하지 않은 입력, 존재하지 않는 리소스 등에 대해 의도된 에러 응답을 반환.
* [ ] 500(서버 에러)이 사용자에게 노출되지 않도록 한다.
* [ ] 에러 응답 본문에 요청이 실패한 이유와 잘못된 조건을 명확히 담는다.
* [ ] 브라우저 화면에서 에러 발생 시 응답 메시지를 사용자에게 보여준다.

### 내 예약 조회

* [ ] 사용자가 자신의 이름으로 본인의 예약 목록을 조회할 수 있다.
* [ ] 사용자 이름이 일치하지 않으면 조회 거부.
...
* [ ] 존재하지 않는 예약이면 거부한다.
* [ ] 사용자 이름이 일치하지 않으면 변경 거부.
* [ ] 이미 지난 예약은 변경할 수 없다.
* [ ] 변경하려는 날짜/시간이 과거이면 거부한다.
* [ ] 변경할 예약 시간이 차 있으면 거부한다.
* [ ] 존재하지 않는 시간 ID로 변경하는 요청을 거부한다.
