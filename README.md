...
##### 회원 기능 목록
- 회원 가입
  - [] 회원가입으로 사용자 정보를 추가할 수 있다.
    - [] 회원 이름은 4글자까지 입력 가능하다.
    - [] 회원 이름을 null, 빈 문자열, 공백으로 요청할 수 없다.
    - [] 이메일은 이메일 형식으로 입력 가능하다.
  - [] 이미 가입된 회원 이름일 경우 예외가 발생한다.
  - [] 이미 가입된 이메일이라면 예외가 발생한다.

- 로그인
  - [] 가입된 회원일 경우 로그인에 성공하고 예약 페이지로 이동 시킨다.
    - [] 존재하지 않는 회원 정보일 경우 예외가 발생한다.
  - [] 인증된 사용자가 아닐 때 다른 페이지 접근 시 로그인 페이지로 이동 시킨다.
