...
### 내 예약 조회·변경·취소 — 사용자 식별 `userId`로 처리
요구사항에는 `자신의 이름으로 예약 목록을 조회한다`는 표현이 있지만,
현재 로그인/세션 기능이 없는 단계에서는 이름이든 ID든
 `이 요청이 실제 본인인지` 검증할 수단이 없다고 생각했다.
```text
userId = 1, 2, 3 ..
→ 누구나 타인의 userId를 추측해서 조회 가능

이름으로 조회 → DB에 동명이인 3명
→ 누구의 예약을 보여줘야 하는지에 대한 로직이 필요
```
두 방식 모두 인증이 부재한 임시 방편이라면,
세션 기반 인증 도입 시 쉽게 교체될 수 있도록
서버 내부 식별자(`userId`)를 미리 반영하는 것이 합리적이라 판단했다.
