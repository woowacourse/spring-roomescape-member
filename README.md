...
- 처음에는 예약 가능한 시간만 반환하는 API를 고민했지만, 클라이언트에서 예약 불가능한 시간도 함께 표현해야 한다고 판단하여 
전체 시간과 예약 가능 여부를 함께 반환하도록 변경했습니다.
- 또한 특정 테마의 시간 목록이라는 의미를 표현하기 위해 `/themes/{id}/times?date=...` 형태로 API를 설계했습니다.
