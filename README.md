...
| TA3 | 서버 오류인 경우                   | 500 Internal Server Error | 예상치 못한 오류가 발생했습니다. |

### 파라미터별 에러 메시지

| ID    | 상황                  | 문제 파라미터명 | 파라미터 에러 메시지            |
|-------|---------------------|---------|------------------------|
| TA2-1 | date 형식이 올바르지 않은 경우 | date    | date의 값이 유효하지 않습니다.    |
| TA2-2 | themeId가 숫자가 아닌 경우  | themeId | themeId의 값이 유효하지 않습니다. |
...
```json
{
  ...
  "parameterErrors": [
    {
      "parameter": "date",
      "message": "date의 값이 유효하지 않습니다."
    },
    {
      "parameter": "themeId",
      "message": "themeId의 값이 유효하지 않습니다."
    }
  ]
}
```
