...
- [ ] 지나간 날짜와 시간으로 예약하면 400을 반환한다.
- [ ] 예약 날짜가 오늘이고 현재 서버 시간 이전의 예약 시간이면 400을 반환한다.
- [ ] 오늘 기준 30일을 초과한 날짜로 예약하면 400을 반환한다.
...
- [ ] 특정 날짜와 테마에 이미 예약된 시간 식별자를 조회한다.
- [ ] 특정 날짜와 테마에 예약이 없으면 빈 목록을 반환한다.

### ReservationRepositoryTest
- [ ] 예약 목록을 페이징 조회한다.
- [ ] 이름으로 예약 목록을 조회한다.
- [ ] 이름에 해당하는 예약이 없으면 빈 목록을 반환한다.

---

## 4. Service Test 기준

이번 미션에서는 Service Test를 기본 체크리스트에 포함하지 않는다.

다음 경우에만 선택적으로 작성한다.
- API 테스트로 실패 원인을 파악하기 어렵다.
- 여러 Repository 결과를 조합하는 분기가 복잡하다.
- 특정 조건에서 저장이 호출되지 않아야 함을 명확히 검증하고 싶다.
- 외부 API(Application Programming Interface), 메일, 결제, 시간, 랜덤 값처럼 제어하기 어려운 의존성이 있다.
