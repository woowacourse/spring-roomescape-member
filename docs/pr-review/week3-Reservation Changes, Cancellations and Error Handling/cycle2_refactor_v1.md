...
- [ ] **3. `DaoTest` 웹 계층 제거**
  - 현재 `@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT)` → 불필요한 웹 컨텍스트 전체 로드
  - `@JdbcTest`로 변경 — JDBC 관련 빈만 로드, DAO 테스트에 적합
- [ ] **4. Tautological test 제거 — Mock 출력을 그대로 검증하는 테스트**
...
- [ ] **4. data.sql 하드코딩 의존 테스트 개선**
  - `ReservationDaoTest` — `findAll`(19), `save`(20L), `delete`(18)
  - `ReservationTimeControllerTest` — `API_예약_시간_조회`, `API_예약_시간_삭제` (13 하드코딩)
  - `ReservationControllerTest` — `이미_예약된_시간이면_400` (`"2026-04-29"`, `time_id=3` 등 data.sql 특정 row 의존)
- [ ] **5. 잘못된 테스트 수정**
  - `ThemeControllerTest.API_예약_시간_조회` — `GET /themes?condition=popular&size=10`으로 요청하고 있으나 인기 테마 엔드포인트는 `/themes/popular`임 → 200이 나와도 다른 엔드포인트를 타는 중
