...
두 파라미터를 모두 생략하면 최근 1주(오늘 기준 7일 전 ~ 1일 전)를 기본값으로 사용한다.

쿼리 파라미터

| 이름          | 필수 | 설명                                             |
|-------------|----|------------------------------------------------|
| `startDate` | 선택 | 집계 시작 날짜 (포함, `yyyy-MM-dd`). 생략 시 최근 1주 기본값 사용 |
| `endDate`   | 선택 | 집계 끝 날짜 (포함, `yyyy-MM-dd`). 생략 시 최근 1주 기본값 사용  |
...
| `400 Bad Request` | `INVALID_PERIOD` | `endDate`가 `startDate`보다 앞선 경우, 또는 둘 중 하나만 제공한 경우 |
...
- 관리자가 테마를 삭제하는 경우, 존재하는 데이터를 삭제하든 존재하지 않는 데이터를 삭제하든 결국 존재하지 않는 데이터이며, 통일성 있게 하기 위해 204 No Content로 통일한다.
- `/themes/popular`가 되는 이유: 인기가 많은 방탈출의 의미를 가져야 하기 때문.
- 이용 가능 시간 조회의 `date` 쿼리를 optional로 둔다. 생략 시 오늘로 해석되어 API 사용이 편하게 한다.
